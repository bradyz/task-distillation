...
class ModuleWorld(object):
    ...
    def __init__(self, name, world, town_map, hero_actor):
    ...
    def _render_traffic_lights(self, surface, list_tl, world_to_pixel, world_to_pixel_width, from_snapshot=False):
        ...
        for tl in list_tl:
            ...
            start = tl.get_transform().location
            end = carla.Location(start.x - v[0] * 6, start.y - v[1] * 6, start.z)
            line = [world_to_pixel(p) for p in [start, end]]
            pygame.draw.lines(surface, color, False, line, 4)
    ...
    def _render_vehicles(self, vehicle_surface, self_surface, list_v, world_to_pixel, from_snapshot=False):
        ...
        for v in list_v:
            ...
            if not from_snapshot and v[0].attributes['role_name'] == 'hero' and False:
    ...
    def render(self, display, snapshot=None):
        ...
        if self.hero_actor is not None:
            ...
            make_image = lambda x: pygame.surfarray.array3d(x)[:,:,0]
            ...
            self.hero_traffic_image = pygame.surfarray.array3d(self.window_traffic_light_surface)
...
class Wrapper(object):
    ...
    def init(cls, player):
        ...
        world_module = ModuleWorld(MODULE_WORLD, world, carla_map, player)
    ...
    def get_observations(cls):
        ...
        birdview = np.swapaxes(np.concatenate(birdview, 2), 0, 1)

        result = cls.world_module.get_hero_measurements()
        result['birdview'] = birdview
